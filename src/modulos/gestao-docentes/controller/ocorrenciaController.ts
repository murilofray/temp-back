...
export class OcorrenciaController {
  ...
  async updateStatus(req: Request, res: Response) {
    try {
      ...
      const resposta = await ocorrenciaService.updateStatus(Number(req.params.id), status);
      ...
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).send({ error: 'Erro ao atualizar o status da ocorrencia.' });
    }
  }
